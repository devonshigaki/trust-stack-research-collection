"""Sim F1a2 driver — SIM_F1A2_PRE_REGISTRATION.md (frozen 2026-09-12).
CS = 264 uniPNs @150Hz; appetitive US = 307 PAM direct @150Hz; rest per Addendum A.
...
N = len(C['roots'])
ORN, SUGAR = POPS['uniPN'], POPS['PAM']  # F1a2: CS=all uniPNs, appetitive US=PAM direct
...
def main():
    import os
    which = os.environ.get('F1A2_SET', '1')
    arms = (['intact_av', 'D_abl', 'S_abl'] if which == '1'
            else ['appetitive', 'OA_abl', 'shuffle'])
    rows = []
    out_csv = f'{OUT}/f1a2_probe_readouts_set' + which + '.csv'
    for arm in arms:
