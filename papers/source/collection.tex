...
gate 4/4). Compound verdict FAIL on the structural arm, MET on the dynamical arm:
global partner permutation fails to abolish function in both species ---
on different metrics and criteria (the fly's abolition index vs.\ a
cortical throughput ratio), not a matched quantitative replication ---
while the single-class structural basis does not transfer. The joint
pattern is the signature the framework's functional-universality claim
predicts: the law recurs, the wiring does not. Reported in the DOCAS
paper (\S{}5.7).
